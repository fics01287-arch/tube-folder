...
| 4단계 | 마무리 다듬기 — 있으면 좋지만 없어도 동작에 지장 없는 항목, 여유 있을 때 처리 | 9/16 완료 + 2/8 구현 완료(검증 대기) (2026-08-30, 8개 전체 작업순서 확정, 1/8 완료·2/8 구현 완료) |
...
| 4. 다듬기 | 다중 선택 + 일괄 이동/삭제 | 중간 | ✅ 완료 (작업순서 2/8, 2026-08-30 구현, 2026-09-10 클로드가 Playwright 자동 클릭 테스트로 검증) — 체크박스 다중선택·Shift+클릭 범위선택·일괄 이동(자기자신·하위폴더·현재부모로는 이동 못하게 막히는 것 포함)·일괄 휴지통 이동·휴지통 안 일괄 복원·Ctrl+Z 실행취소·토스트 "다시 실행" 버튼까지 전부 실제 클릭 시뮬레이션으로 통과 확인. 산들이 직접 클릭 테스트할 필요 없음 |
| 4. 다듬기 | 무료 한도 도달 시 업그레이드 안내 팝업 일원화(버그 수정) | 낮음~중간 | ✅ 완료 (2026-09-10) — App.tsx 7곳 + 유튜브 페이지 단일 추가 미니팝업에 이어, 산들이 실제 화면(재생목록 가져오기 150개 한도 스크린샷)으로 제보한 8번째 누락 지점(`content.ts`의 `finishImport()`, 독립 코드 경로라 앞선 수정에서 누락)까지 `showLicenseLimitPopup()` 공용 함수로 통일. Playwright로 miniPopup.ts를 실제 브라우저에 올려 팝업 교체·PRO 버튼 클릭까지 검증 |
| 4. 다듬기 | 재생(정렬순/무작위 재생·다중선택 재생·1회/무한재생) | 중간 | ✅ 완료 (2026-09-10) — 정렬된 순서/무작위로 순차 재생, 폴더 전체 재생과 다중선택 재생, 1회재생/무한반복 선택 다이얼로그. 유튜브 탭에서 영상이 끝나면 자동으로 다음 곡으로 이동(하드 네비게이션). 단위 테스트 + Playwright 브라우저 테스트로 전부 검증 |
| 4. 다듬기 | F2/Delete 단축키 | 낮음~중간 | ⬜ 미착수 (작업순서 3/8, 2026-08-30 산들 착수 승인) |
| 4. 다듬기 | 우클릭 클립보드(복사·잘라내기·붙여넣기) | 높음 | ⬜ 미착수 (작업순서 4/8, 2026-08-30 산들 착수 승인) |
| 4. 다듬기 | 폴더 위로 드래그해 바로 이동 | 중간~높음 | ⬜ 미착수 (작업순서 5/8, 2026-08-30 산들 착수 승인) |
...
- [x] **다중 선택 + 일괄 이동/삭제/복원** — 완료 (2026-08-30 구현, 2026-09-10 클로드가 Playwright 자동 클릭 테스트로 검증 완료, 작업순서 2/8)
  ...
  - **검증**: `tsc --noEmit` 통과(체크박스 삽입 과정에서 JSX 중괄호/괄호 짝이 어긋난 실수 1건 발견·수정). **2026-09-10 추가 검증**: 로컬 정적 서버 + Playwright로 실제 클릭·키보드 입력을 시뮬레이션해 다음을 전부 확인 — ①체크박스로 개별 선택 시 "N개 선택됨" 툴바 노출 ②Shift+클릭 범위 선택(A→C 3개 한 번에 선택) ③📁 일괄 이동으로 목적지 폴더 선택 시 정확히 이동, 자기 자신·하위 폴더·현재 부모 폴더는 이동 다이얼로그에서 비활성화됨(사이클 방지 확인, 중첩 폴더로 별도 시나리오 테스트) ④이동 직후 Ctrl+Z → 정확히 원상복구 + "…취소됨" 토스트 ⑤토스트의 "다시 실행" 버튼 → 이동이 다시 적용됨 ⑥🗑 일괄 휴지통 이동(확인 단계 포함) → 선택 항목 전부 휴지통으로 ⑦휴지통 폴더 진입 후 체크박스+Shift 범위선택으로 전체 선택 시 툴바가 ↩ 일괄 복원 버튼으로 정확히 바뀜, 클릭 시 전부 원래 위치로 복원. 앱 코드 결함은 발견되지 않음(테스트 스크립트 자체의 선택자 실수 2건만 발견해 수정 — 폴더 정렬이 이름의 코드값 순이 아니라 `localeCompare(..,'ko')` 기준이라 렌더 순서가 기대와 달랐던 것이 원인).
...
- [ ] **F2(이름변경)/Delete(삭제) 단축키** (신설 2026-08-29, 매뉴얼-실제 기능 불일치 조사 중 도출한 추천 기능 3순위) — **작업순서 3/8, 2026-08-30 산들 착수 승인**
  ...
  - 전제: 착수 승인 완료(2026-08-30, 산들 확정). 다중 선택 항목 완료 후 착수 권장(선택 개념이 먼저 있어야 자연스럽게 연결됨).
  - 복잡도: 낮음~중간(다중 선택 이후 기준)

- [ ] **폴더 위로 드래그해 바로 이동** (신설 2026-08-30, 매뉴얼-실제 기능 불일치 조사에서 발견 — 매뉴얼 7·10장 "정렬 기준과 무관하게 항상 드래그로 이동 가능"이 실제로는 안 되던 항목) — **작업순서 5/8, 2026-08-30 산들 착수 승인**
  - 배경: 지금 드래그(`@dnd-kit`)는 같은 폴더 안 순서 변경만 지원 — 다른 폴더로 옮기려면 📁 버튼 → 이동 대상 선택 모달을 거쳐야 함. 탐색기처럼 항목을 폴더 아이콘 위에 직접 끌어다 놓으면 그 안으로 들어가는 동작을 추가하는 것.
  - 내용: `@dnd-kit`의 드롭 대상 판정에 "폴더 위에 드롭" 케이스를 새로 추가 — 기존 순서변경(형제 항목 사이 삽입선)과 폴더-진입(항목 위에 정확히 겹침) 두 판정이 서로 다른 시각 피드백(삽입선 vs 폴더 하이라이트)으로 구분되게 처리 필요. 실제 이동 처리는 기존 `moveNode`(사이클 방지 포함) 재사용, 실행취소도 기존 스냅샷 방식 그대로 적용될 것으로 예상.
  - 전제: 착수 승인 완료(2026-08-30, 산들 확정). 기존 순서변경 드래그와 동작이 겹치지 않게 구분하는 부분이 핵심 검증 포인트.
  - 복잡도: 중간~높음(드롭존 판정 로직 신규)

- [ ] **우클릭 클립보드(복사·잘라내기·붙여넣기)** (신설 2026-08-29, 매뉴얼-실제 기능 불일치 조사에서 발견 — 매뉴얼에 있던 복사/잘라내기/붙여넣기가 실제로는 없던 항목) — **작업순서 4/8, 2026-08-30 산들 착수 승인**
  - 배경: 지금은 "이동"(`moveNode`)만 있고 "복사"(원본은 그대로 두고 똑같은 사본을 만드는 것) 기능 자체가 코드에 없음. 탐색기의 Ctrl+C/X/V에 해당하는 동작을 우클릭 메뉴로 제공하는 것.
  - 내용: ①다중 선택 기능(작업순서 2/8)의 선택 상태를 재사용해 "복사"/"잘라내기" 시 선택된 항목들을 메모리상 클립보드 상태(무엇을 복사/잘라냈는지)로 저장 → ②"붙여넣기" 시 복사면 새 `duplicateNode`(신규 함수, 폴더는 하위 트리 전체를 깊은 복사) 호출, 잘라내기면 기존 `moveNode` 호출 → ③붙여넣을 폴더에 동명 항목이 있으면 기존 `uniqueName` 관례(이름 뒤에 번호)로 자동 처리.
  - 전제: 다중 선택(작업순서 2/8) 완료 후 착수하는 것이 자연스러움(선택 상태를 그대로 재사용). 착수 승인 완료(2026-08-30, 산들 확정).
  - 복잡도: 높음("무엇을 잘라냈는지 기억"하는 클립보드 상태 신설 + `duplicateNode` 깊은 복사 로직 신규 설계 필요)
...
- [x] **무료 한도 도달 시 업그레이드 안내 팝업 일원화(버그 수정)** — 완료 (2026-09-10)
  - 배경: 이번 세션(2026-09-10)에 진행한 버그 수정 — `LicenseLimitNotice.tsx` 신설(무료 한도 걸릴 때 안내 팝업, App.tsx 7곳에 연결) · 유튜브 페이지에서 단일 동영상 추가 시 한도 걸리면 background↔content 메시징(`TF_SHOW_LICENSE_LIMIT`)으로 미니 팝업 안내 · "PRO 알아보기" 클릭 시 매니저 탭의 라이선스 패널까지 자동으로 열리도록 딥링크(`TF_OPEN_MANAGER`+`openManagerAndShowLicense()`+`?openLicense=1`) 구현 · `chrome://extensions` "오류" 목록에 정상 처리된 한도초과 상황이 진짜 버그처럼 잡히던 문제를 `console.error`→`console.warn`으로 수정.
  - **8번째 누락 지점 발견 및 수정(2026-09-10 늦게, 산들 제보)**: 산들이 실제 화면(재생목록 가져오기 중 150개 한도 도달 스크린샷)으로 "재생목록 가져오기는 유료전환 메시지가 안 뜬다"고 제보 — 위 7곳과 달리 유튜브 페이지에서 "재생목록 가져오기"로 영상을 가져오다 한도에 걸리는 경우는 `content.ts`의 `finishImport()`가 독립된 코드 경로라 수정에서 빠져 있었음(그냥 `throw new Error(...)`만 해서 "가져오기 확인" 창에 업그레이드 버튼 없는 텍스트만 인라인으로 떴음). `showLicenseLimitPopup()` 공용 함수로 추출해 이 지점도 "PRO 알아보기" 버튼이 있는 동일 안내 팝업이 뜨도록 수정.
  - **검증**: Playwright로 `miniPopup.ts`를 실제 브라우저에 올려 놓고 "확인 팝업 → 한도 팝업 교체 → PRO 버튼 클릭"까지 직접 검증(리액트 매니저 탭과 달리 이 흐름은 유튜브 실제 페이지 DOM이 있어야 완전한 종단 검증이 되므로, 이번 검증은 "팝업 교체·전환 로직 자체"까지만 확인한 것 — 실제 유튜브 페이지에서의 최종 확인은 산들 몫). `npm run build`(tsc --noEmit + 확장 3종) 통과. 빌드된 `dist/content.js`를 산들 컴퓨터에 직접 반영해 `npm run build` 없이도 바로 적용됨.
  - 🔴 치명적·🟡 중요 결함 0건.
  - 복잡도: 낮음~중간

- [x] **재생(정렬순/무작위 재생·다중선택 재생·1회/무한재생)** (신설 2026-09-10, 산들 요청 — "정렬되어 있는데로(이름순, 날자순, 직접순서 등)순차로 재생하는 기능, 무작위로 재생하는 기능 추가 / 파일을 다중선택해서 재생하는 기능 추가 / 재생목록으로 선택한 파일을 1회만 재생, 무한재생을 선택하는 기능 추가") — 완료
  - 배경: 지금까지는 영상을 한 번에 하나씩만 클릭해서 재생할 수 있었음. 폴더 안 영상 전체(또는 다중 선택한 일부)를 정렬 순서대로 또는 무작위로, 한 번만 또는 끝까지 반복해서 순차 재생하는 기능을 새로 추가.
  - **구현 내용**:
    - `src/shared/playbackQueue.ts`(신규) — 재생 큐의 "브레인" 역할, `App.tsx`가 들고 있는 본체(`tubefolder_v1`)와는 무관한 "지금 이 브라우저에서 재생 중인 것" 전용 로컬 상태를 별도 키(`tubefolder_playback_queue`)로 저장(storage.ts와 동일한 `chrome.storage.local`↔`localStorage` 이중 폴백 패턴). `getQueueState`/`setQueueState`(큐 읽기·쓰기) · `buildQueueItems`(videoId 없는 영상은 건너뜀, handleVideoClick의 기존 null 체크와 동일 원칙) · `shuffle`(Fisher-Yates, 재생 시작 시 한 번만 섞고 무한재생으로 순환할 때 다시 섞지 않음 — 일반 미디어 플레이어의 "셔플" 관례) · `computeNextIndex`(다음 인덱스 계산, 1회재생 끝이면 null·무한반복이면 0으로 순환) · `isCurrentPageInQueue`(지금 보고 있는 유튜브 페이지가 이 큐의 "현재 항목"과 실제로 일치하는지 확인 — 사용자가 재생 중간에 직접 다른 영상으로 이동했거나 오래된 큐가 남아있을 때 엉뚱하게 다음 곡으로 넘기는 사고를 막는 안전장치).
    - `src/content/content.ts` — `initPlaybackQueueWatcher()`(모듈 로드 시 호출) 신규: 유튜브 시청 페이지의 `<video>` 엘리먼트를 폴링으로 찾아 `'ended'` 이벤트를 건다(`watchForVideoEnded`). 끝나면 `handleQueueVideoEnded()`가 큐 상태를 읽어 `isCurrentPageInQueue`로 "정말 이 큐의 현재 영상이 끝난 게 맞는지" 확인 후 `computeNextIndex`로 다음 영상을 계산해 **하드 네비게이션**(`window.location.href = ...`)으로 이동 — SPA 내부 라우팅에 의존하지 않아 다음 영상 페이지에서 content script가 매번 깨끗하게 다시 주입됨. 더 재생할 게 없으면(1회재생 모드에서 끝에 도달) 큐를 지우고 아무 데도 이동하지 않음.
    - `src/manager/PlaybackOptionsDialog.tsx`(신규) — 재생 시작 전 순서(정렬된 순서대로/무작위)·반복(1회만/끝까지 후 무한 반복)을 고르는 모달, 기존 `MoveDialog` 등과 같은 `.tf-sync-overlay`/`.tf-sync-panel` 틀 재사용.
    - `src/manager/App.tsx` — `folderVideoNodes`(현재 폴더의 직계 영상만, 이미 정렬된 `children`에서 추출 — "정렬되어 있는대로 순차 재생" 요청을 그대로 만족) · 폴더 요약문(`tf-folder-summary`) 옆 "▶ 폴더 재생" 버튼(휴지통 안이거나 영상이 0개면 숨김) · 다중 선택 툴바의 "▶ 재생" 버튼(선택 중 영상이 1개라도 있어야 노출, 체크한 순서가 아니라 화면에 보이는 정렬 순서로 큐를 만듦) · `handlePlaybackStart(order, repeat)`: 순서 적용(무작위면 `shuffle`) → `buildQueueItems` → **첫 영상만 `handleVideoClick`과 동일하게 `window.open()`으로 실제 유튜브 탭을 열고**(임베드 재도입 금지, 2026-09-08 "오류 152-4" 결론 유지) → 큐 상태 저장(비동기, 클릭 이벤트 핸들러 안에서 팝업 차단을 피하려 `window.open`을 먼저 동기 호출한 뒤 fire-and-forget으로 이어감) → 다이얼로그 닫고 선택 해제. 이어보기 위치(`lastPosition`의 `t=` 파라미터)는 큐 재생에는 적용하지 않음(여러 영상을 순서대로 넘기는 것과 "지난번 멈춘 자리부터"는 다른 기능이라 섞으면 혼란).
    - `src/manager/App.css` — `.tf-folder-play-btn`(요약문 옆 작은 버튼) · `.tf-playback-option-group`(다이얼로그의 순서/반복 fieldset, 옅은 구분선) 신규 클래스 추가. `.tf-btn-primary`는 기존에 이미 있어 재사용.
  - **검증**: `npm run build`(tsc --noEmit + 확장 3종 빌드) 통과. ①`playbackQueue.ts`는 `npx tsx`로 순수 로직 단위 테스트(8개 assertion) ②`content.ts`의 감시 로직은 esbuild로 실제 파일을 번들링해 Playwright(headless Chromium)에 올리고 유튜브 요청을 로컬에서 가로채(`page.route`) 검증 — DOM에 `<video>`가 나타난 뒤에만 `ended` 감지·정상 진행(다음 영상 URL로 실제 이동 확인)·이 큐와 무관한 페이지에서는 안전장치로 아무 일도 안 일어남·1회재생 모드 마지막 항목에서 큐가 지워지고 이동 없음·무한반복 모드 마지막 항목에서 처음으로 순환, 총 5개 시나리오 전부 통과 ③React UI(`App.tsx`)는 빌드된 `dist/`를 로컬 정적 서버로 띄우고 Playwright로 실제 클릭 검증 — "▶ 폴더 재생" 버튼 노출+클릭 시 다이얼로그 노출, 기본값(정렬순/1회)으로 시작 시 첫 영상이 올바른 URL로 열리고 큐가 화면 순서(A,B,C) 그대로 저장됨, "무작위로 재생" 선택 시 실제로 순서가 섞임(5회 반복 실행으로 매번 다른 순열 나오는 것 확인), "무한 반복" 선택 시 `repeatMode: 'loop'`로 저장됨, 취소 버튼은 큐·`window.open` 둘 다 변화 없음, 다중 선택에서 폴더만 선택하면 "▶ 재생" 버튼이 숨음, C를 먼저·A를 나중에 체크해도 큐는 화면 정렬 순서(A,C)를 따름, 재생 시작 후 선택이 해제됨, 휴지통 폴더 안에서는 영상이 있어도 "▶ 폴더 재생" 버튼이 숨음 — 총 7개 시나리오 전부 통과. 앱 코드 결함은 발견되지 않음.
  - **검증 한계**: 로컬 정적 서버(`localStorage` 폴백) 기준 검증 — 실제 확장 컨텍스트(`chrome.storage.local`)·실제 유튜브 탭에서 영상이 끝났을 때 정말로 다음 탭이 열리는지는 산들의 실사용 확인을 권장. 여러 개를 동시에 여러 탭에서 재생하는 경우(큐가 탭마다 아니라 브라우저 전체에 하나뿐이라 마지막에 시작한 재생이 우선됨)는 이번 범위에서 의도적으로 다루지 않음 — 1인 사용자, 한 번에 하나씩 순차 재생하는 일반적 사용 패턴 기준으로 설계.
  - 🔴 치명적·🟡 중요 결함 0건. 🟢 남은 범위(이번에 의도적으로 제외, 필요 시 별도 항목): ①이전/다음 곡 수동 건너뛰기 버튼(지금은 자동 진행만 있음) ②재생 중 큐 진행 상황을 매니저 탭에서 보여주는 UI(지금은 재생 시작 시점에만 다이얼로그가 뜨고, 이후 진행 상황은 유튜브 탭 안에서만 조용히 진행됨).
  - 복잡도: 중간
...
- **다음 백로그 항목(F2/Delete 단축키, 작업순서 3/8) 착수 대기** (2026-09-10 기록, 아직 미착수) — 4단계 섹션에 "⬜ 미착수" 상태로 이미 등록돼 있고 산들 착수 승인도 완료된 상태(2026-08-30). 다중 선택(2/8) 완료·검증까지 끝나서 선행조건은 충족됨. (같은 날짜에 있던 "라이선스 한도 버그 수정 내역 기록" 항목은 2026-09-10에 4단계 섹션에 새 항목으로 기록 완료돼 이 목록에서 제거함.)
