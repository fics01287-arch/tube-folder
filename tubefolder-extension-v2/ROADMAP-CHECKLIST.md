...
| 3단계 | 고급 기능 — 외부 서비스 연동 등 복잡도·리스크가 높아 핵심 기능이 안정된 뒤 진행 권장 | 0/5 완료 |
...
| 3. 고급 기능 | 모바일 자동 동기화(구글드라이브) | 높음 | 🔧 진행중 (2026-07-23) — 코드·검증 완료, 산들의 OAuth 클라이언트 ID 발급 + 실확장 검증 대기 |
| 3. 고급 기능 | PWA(휴대폰 매니저)에 동기화 붙이기 | 중간 | ⬜ 미착수 (신설 2026-07-23) |
...
- [ ] **모바일 자동 동기화 (구글 드라이브)** — 🔧 진행중 (2026-07-23, 코드·목검증 완료 / 산들 계정 작업 대기, 체크포인트 커밋 `8b82ee6`)
  ...
  - **남은 작업**: ①산들 계정 작업 — Google Cloud OAuth 클라이언트 ID 발급(README.md "동기화 사용 준비" 단계별 안내 작성됨) ②발급 후 실제 확장 로드 상태에서 종단 검증(진짜 구글 OAuth 창·Drive API) ③이후 완료 판정.
  - 복잡도: 높음

- [ ] **PWA(휴대폰 매니저)에 동기화 붙이기** (신설 2026-07-23, "모바일 자동 동기화" 설계 승인 시 분리 확정)
  ...
  - 준비된 것: 같은 GCP 프로젝트에서 웹용 클라이언트 ID만 추가 발급하면 같은 appDataFolder를 읽을 수 있고, `SyncBackend` 인터페이스에 GIS 기반 어댑터를 추가하는 형태로 확장 가능. 병합 로직(`sync/merge.ts`)은 chrome 의존성이 없어 그대로 재사용.
  - 전제: "모바일 자동 동기화" 항목 완료(실확장 검증) 후 착수.
  - 복잡도: 중간
