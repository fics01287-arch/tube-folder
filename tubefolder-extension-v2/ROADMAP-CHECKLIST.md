...
작성일: 2026-07-20 (최종 갱신: 2026-07-27)
...
| 3단계 | 고급 기능 — 외부 서비스 연동 등 복잡도·리스크가 높아 핵심 기능이 안정된 뒤 진행 권장 | 2/5 완료 |
| 4단계 | 마무리 다듬기 — 있으면 좋지만 없어도 동작에 지장 없는 항목, 여유 있을 때 처리 | 3/6 완료 |
...
- [ ] **PWA(휴대폰 매니저)에 결제 확인 붙이기** (신설 2026-07-21, 앱개발지침 §7 "멀티 플랫폼 배포 시 주의" 반영)
  ...
  - **조사 결과 (2026-07-27, 산들 결정 필요)**: ExtPay.js 공식 문서(GitHub `Glench/ExtPay` README) 확인 결과, `extpay.getUser()` 등 API는 `webextension-polyfill` 기반으로 브라우저 확장 컨텍스트(`chrome.storage`·`chrome.runtime`) 전용으로 설계돼 있고, 일반 웹페이지(PWA)에서 바로 호출할 수 있는 별도의 공개 REST 엔드포인트는 문서상 확인되지 않음. 즉 "결제 연동 구현"에서 이미 선택한 ExtensionPay를 PWA에 그대로 붙이는 것은 현재로선 지원되지 않아 보임 — 착수 시점에 아래 중 하나를 산들이 선택해야 진행 가능: ①ExtensionPay 측에 웹용 조회 방법을 직접 문의 ②PWA에서는 결제 확인 없이 무료 기능만 제공(동기화·한도 해제는 확장 전용으로 한정) ③다른 서비스(Fungies.io 등) 병행 검토. 착수 전 반드시 산들과 논의 필요.
  - 복잡도: 중간
...
- [x] **휴지통 보존기간 설정·자동 비우기** — 구현 완료 (2026-07-27)
  - 착수 전 반영 필요했던 5가지(앱개발지침 §5 "삭제·보관정책 UX 투명성 원칙") 모두 반영: ①보관기간·자동 영구삭제 시점을 휴지통 화면에 상시 배너로 안내 ②보관기간 조정 옵션 + "자동 삭제 없음" 옵션 ③보관기간 변경은 기존 항목에도 소급 적용 기본값 ④보관기간을 줄여 기존 항목이 즉시 영구삭제될 수 있는 경우 영향받는 항목 수를 알리고 확인받기 ⑤(아래 참고, 부분 반영) 초기 사용 기간 삭제 시점 안내.
  ...
  - 🔴 치명적·🟡 중요 결함 0건. 🟢 참고: ①자동 비우기가 "열 때 확인" 방식이라 크롬 확장을 며칠씩 안 열면 그만큼 정리가 늦어짐(데이터 유실은 아님) — 더 즉각적으로 하려면 `chrome.alarms` 주기 실행으로 승격 가능 ②"초기 사용 기간 동안 삭제 시점에도 동일 안내 노출"은 기존 삭제 확인 문구에 보관기간 한 줄만 추가하는 것으로 간단히 반영했고, "신규 사용자만 별도로 더 강조" 같은 정교한 처리는 하지 않음.
  - 복잡도: 낮음
