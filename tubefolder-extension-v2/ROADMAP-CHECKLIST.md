...
작성일: 2026-07-20 (최종 갱신: 2026-07-29)
...
| 3단계 | 고급 기능 — 외부 서비스 연동 등 복잡도·리스크가 높아 핵심 기능이 안정된 뒤 진행 권장 | 3/6 완료 |
| 4단계 | 마무리 다듬기 — 있으면 좋지만 없어도 동작에 지장 없는 항목, 여유 있을 때 처리 | 4/7 완료 |
| 5단계 | 출시 준비 — 앱 기능이 다 완성된 뒤에 진행하는 마무리 작업 | 0/5 완료 |
...
| 5. 출시 준비 | 버그 제보 채널 마련 | 낮음 | ⬜ 미착수 (신설 2026-07-28) |
...
- [ ] **승인 기반 무료 라이선스 구현** (신설 2026-07-21, 앱개발지침 §7 반영)
  ...
  - **주의 — 위 "유료→무료 전환 스위치"(바로 위 항목, 완료됨)와는 다른 기능**: 그 항목은 켜면 **전체 사용자**가 무료가 되는 전역 스위치이고, 이 항목은 개발자가 지정한 **특정 인원만** 무료로 쓰게 하는 개별 승인 기능이다 — 서로 대체 관계 아님, 별도로 구현 필요.
  - 복잡도: 중간
...
- [ ] **버그 제보 채널 마련** (신설 2026-07-28, Chat 세션 "유료 판매 리스크 대응" — CLAUDE.md "YouTube DOM 변경 대응 원칙" 반영)
  - 배경: YouTube 페이지 구조 변경을 개발자가 먼저 감지하기 어려운 경우가 많아, 사용자 제보가 가장 빠른 변경 감지 수단이 될 수 있음.
  - 내용: 이메일 또는 오픈카톡방 등 간단한 제보 채널을 마련해 매뉴얼(5단계 사용자 매뉴얼 작성 항목과 연계)·설정 화면에 안내한다.
  - 전제: 5단계 "사용자 매뉴얼 작성" 착수 시점에 함께 반영하면 중복 작업을 줄일 수 있음.
  - 복잡도: 낮음
